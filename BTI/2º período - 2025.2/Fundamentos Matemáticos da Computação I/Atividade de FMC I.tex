...
\begin{document}
	...
	\[
	(\forall A \subset \mathbb{R})[(\exists M \in \mathbb{R})(\forall x \in A)[x \leq M] \implies (\forall c > 0)[\sup(c \cdot A) = c \cdot \sup(A)]]
	\]
	
	Seja $A \subset \mathbb{R}$. Então existe $M$ real tal que, para todo $x \in A$, temos $x \leq M$. Seja, também, $c$ um número real tal que $c > 0$. Vamos demonstrar que $\sup(c \cdot A) = c \cdot \sup(A)$.
\end{document}
