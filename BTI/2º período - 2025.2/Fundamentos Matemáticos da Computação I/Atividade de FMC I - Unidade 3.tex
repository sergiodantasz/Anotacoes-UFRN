...
\begin{document}
	...
	Por ser cotada superiormente e monótona, temos que $(s_n)_n$ é convergente, pelo Teorema da Convergência Monótona. \blacksquare
	
	\section*{Questão 3: Encontre o Supremum e o Infimum do conjunto de todos os reais que podem ser representados na forma $2^{-p} + 3^{-q} + 5^{-r}$ em que $p$, $q$ e $r$ são inteiros positivos.}
	...
	Seja $x \in A$. Por definição, existem inteiros $p$, $q$ e $r$ tais que $x = \frac{1}{2^p} + \frac{1}{3^q} + \frac{1}{5^r}$. Como as bases são positivas e os expoentes são inteiros, cada fração é positiva e, consequentemente, a soma dessas três frações também é positiva. Logo, $x > 0$.
	
	Ou seja, com isso nós sabemos que $A$ é limitado inferiormente e 0 é uma cota inferior. Dessa forma, $\inf(A) \geq 0$. Vamos investigar se $\inf(A) = 0$ ou $\inf(A) > 0$.
	
	Testemos o comportamento quando aumentamos os expoentes. Vamos calcular o limite quando os expoentes tendem ao infinito:
	\[
	\lim_{p,q,r \to \infty}{\left( \frac{1}{2^p} + \frac{1}{3^q} + \frac{1}{5^r} \right)} = 0 + 0 + 0 = 0
	\]
	
	Como os valores do conjunto $A$ podem ser arbitrariamente pequenos (próximos de zero), não existe nenhuma cota inferior maior que 0. Mas note que 0 não pertence ao conjunto, pois a soma nunca dá exatamente zero.
	
	Portanto, $\inf(A) = 0$. \blacksquare
    ...
    Como $3 \mid a$ e $3 \mid b$, segue que $\mdc(a,b) \geq 3$, contradizendo a hipótese de que a fração $\frac{a}{b}$ é irredutível. Logo $\sqrt{6}$ é irracional.
