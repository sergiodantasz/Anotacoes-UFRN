...
= \mathcal{U}^C \cup B^C \cup C^C \qquad (\text{Lei de De Morgan}) \\
...
$$
\displaylines{
(X^C \cup Y)^C \qquad (\text{Expressão inicial}) \\
= X \cap Y^C \qquad (\text{Lei de De Morgan e } {X^C}^C = X) \\
= X \setminus Y \qquad (A \cap B^C = A \setminus B) \\
= 1 \cup 4 \qquad (\text{Substituição}) \\
}
$$
