...
$$
\displaylines{
A \cup (B \cap C^C) \cup (A^C \cap B^C \cap C^C) \cup C \qquad (\text{Equação inicial}) \\
(A \cup B) \cap (A \cup C^C) \cup (A^C \cap B^C \cap C^C) \cup C \qquad (\text{Distributividade}) \\
(A \cup B) \cap (A \cup C^C) \cup (A \cup B \cup C)^C \cup C \qquad (\text{Lei de De Morgan}) \\
(A \cup B) \cap (A \cup C^C) \cup \mathcal{U}^C \cup C \qquad (A \cup B \cup C = \mathcal{U}) \\
(A \cup B) \cap (A \cup C^C) \cup \emptyset \cup C \qquad (\mathcal{U}^C = \emptyset) \\
(A \cup B) \cap (A \cup C^C) \cup C \qquad (X \cup \emptyset = X) \\
(A \cup B) \cap [A \cup (\mathcal{U} \setminus C)] \cup C \qquad (X^C = \mathcal{U} \setminus X) \\
(A \cup B) \cap [A \cup ((A \cup B \cup C) \setminus C)] \cup C \qquad (A \cup B \cup C = \mathcal{U}) \\
(A \cup B) \cap [A \cup (A \cup B)] \cup C \qquad (\text{Diferença}) \\
(A \cup B) \cap (A \cup B) \cup C \qquad (X \cup X = X) \\
(A \cup B) \cup C \qquad (X \cap X = X) \\
A \cup B \cup C \qquad (\text{Associatividade})
}
$$
...
$$
\displaylines{
[(A^C \cap B \cap C) \cup (A \cap B \cap C)]^C \qquad (\text{Equação inicial}) \\
[((B \cup C) \cap B \cap C) \cup (A \cap B \cap C)]^C \qquad (A \cup B \cup C = \mathcal{U}) \\
[(B \cap C) \cup (A \cap B \cap C)]^C \qquad (X \cap (X \cup Y) = X) \\
(B \cap C)^C \cap (A \cap B \cap C)^C \qquad (\text{Lei de De Morgan}) \\
B^C \cup C^C \cap A^C \cup B^C \cup C^C \qquad (\text{Lei de De Morgan}) \\
(A \cup C) \cup (A \cup B) \cap (B \cup C) \cup (A \cup C) \cup (A \cup B) \qquad (A \cup B \cup C = \mathcal{U}) \\
A \cup B \cup C \qquad (\text{União e interseção}) \\
}
$$
