...
Considere os conjuntos $A = \{ 1, 2, 3 \}$, $B = \{4, 5, 6\}$ e $C = \{6, 7\}$. Aplicando as operações de união e interseção, obtém-se o seguinte resultado:

$$
\displaylines{
A \cup (B \cap C) \neq (A \cup B) \cap C \\
\therefore \{1,2,3\} \cup (\{4,5,6\} \cap \{6,7\}) \neq (\{1,2,3\} \cup \{4,5,6\}) \cap \{6,7\} \\
\therefore \{1,2,3\} \cup \{6\} \neq \{1,2,3,4,5,6\} \cap \{6,7\} \\
\therefore \{1,2,3,6\} \neq \{6\} \\
}
$$
