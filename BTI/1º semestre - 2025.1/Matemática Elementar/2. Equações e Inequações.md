...
Como a inequação deve ser menor ou igual a zero, então consideraremos apenas os valores de $x$ que satisfaçam tal. Pelo quadro de sinais, vamos ter $x \leq -2$ ou $1 < x \leq 2$ ou $x > 4$.

Portanto, o conjunto solução dessa inequação é dado por $S = (-\infty,-2] \cup (1,2] \cup (4,+\infty)$.
